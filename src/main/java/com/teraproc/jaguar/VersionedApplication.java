...
public class VersionedApplication {
    ...
    if (checkIfParamVersion(args)) {
      try {
        if (LONG_VERSION.equals(args[0])) {
          System.out.println(
              "The application info is: \n" + readVersionFromClasspath(
                  "application.properties", false));
        } else if (SHORT_VERSION.equals(args[0])) {
          System.out.println(
              readVersionFromClasspath("application.properties", true));
        }
      } catch (IOException ex) {
        System.out.println(
            "The application.properties file not found version is undefined.");
      }
      return true;
    }
    return false;
  ...
  private boolean checkIfParamVersion(String[] args) {
    return args.length == 1 && (LONG_VERSION.equals(args[0]) || SHORT_VERSION
        .equals(args[0]));
  }

  private String readVersionFromClasspath(String fileName, boolean onlyVersion)
      throws IOException {
